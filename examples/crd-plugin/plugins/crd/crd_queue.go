...
var (
	...
	queueExample      = workqueue.NewRateLimitingQueue(workqueue.NewItemExponentialFailureRateLimiter(time.Second*5, time.Minute))
	queueExampleEmbed = workqueue.NewRateLimitingQueue(workqueue.NewItemExponentialFailureRateLimiter(time.Second*5, time.Minute))
)
...
func workforever(plugin *Plugin, queue workqueue.RateLimitingInterface, informer cache.SharedIndexInformer, stopCH chan struct{}) {
	for {
		...
		func(key string) {
			...
			if !exists {
				plugin.Log.Infof("Object named %s (%s) deleted from queue", name, item)
			} else {
				plugin.Log.Infof("Got most up to date version of '%s/%s' (%s). Syncing...", namespace, name, item)
			}
...
func exampleCrdEnqueue(obj interface{}) {
